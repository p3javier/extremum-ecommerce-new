...
setTimeout(() => {
  ...
  console.log("prueba", typeof adminController.getAdminUsers);

  //Add order to the queue
  app.post("/api/orders", orderController.createOrder);

  app.post("/api/my-orders", orderController.readOrders);

  app.get("/api/products/sku/:sku", productsController.findBySku);
  //Read the user's session.
  app.post("/api/users/id", userController.readUserData);

  //Add a item to cart.
  app.put("/api/users/cart", userController.addProductToCart);

  //Remove a item from the cart.
  // Use request parameter to remove item from cart since you are looking a specific item in cart.
  app.put(
    "/api/users/cart/delete-product",
    userController.deleteProductFromCart
  );

  //When user login
  app.post("/api/users", userController.login);
  ...
  app.get("/api/products", productsController.readProducts);
...
app.post("/send", (req, res) => {
  ...
  var smtpTransport = nodemailer.createTransport({
    ...
    auth: {
      user: "javiperezfc",
      pass: "ejsenynoqlwnpwme",
    },
  });

  var mailOptions = {
    from: data.email,
    to: "javiperezfc@gmail.com",
